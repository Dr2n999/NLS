...
```{r, eval = F}
d1 <- ggplot(df_temp, aes(x, y)) +
  geom_point() +
  stat_function(fun = function(x) w0[1] * exp(-w0[2] * x) * cos(w0[3] * x + w0[4]) + w0[5], col = "red", lty = 2) +
  theme_bw()
ggsave(here("result", "poor.fit.png"), plot=d1, dpi=300)
```

```{r, echo = F}
knitr::include_graphics(here("result", "poor.fit.png"))
```  
...
d2 <- ggplot(df_temp, aes(x, y)) +
  ...
  theme_bw()
ggsave(here("result","good.fit.png"), plot=d2, dpi=300)
```


```{r, echo = F}
knitr::include_graphics(here("result", "good.fit.png"))
```  
